...
namespace kh2mset
{
	public partial class MainForm : Form
	{
		...
		List<string> labels_EC1 = new string[] {
			...
			/*9*/"Model state: Force Animation Time)",
			/*10*/"Damage",
			/*11*/"Combo",
			/*12*/"Attack label: Keyblade Trail",
			/*13*/"Attack label: Glide Helper",
			/*14*/"Attack label: Enable RCs",
			/*15*/"Attack label: 0F (Unknown)",
			/*16*/"Special AI Mobility",
			/*17*/"Target identificatier (AI Combo 1)",
			/*18*/"Model state: AI Combo 2",
			/*19*/"Model state: Disable forced i-frames",
			/*20*/"Internal-model reaction command activator",
			/*21*/"Model state: 015 (Unknown)",
			/*22*/"Model state: Rotate to Target 2",
			/*23*/"DMY TEXA displayer",
			/*24*/"Model state: no gravity (2)",
			/*25*/"External model reaction command activator",
			/*26*/"Model state: 01A (Unknown)",
			/*27*/"Model state: Intangible",
			/*28*/"Rotation to target",
			/*29*/"Stun state: Forcefully Grounded",
			/*30*/"Stun state: 01E (Unknown)",
			/*31*/"Stun state: 01F (Unknown)"
		}.ToList();
		
		List<string> labels_EC1_explanations = new string[] {
			/*0*/"Interpolates animation back into Idle animation at start time. Always place on player/party's idle, walk, and run.",
			...
			/*7*/"Unknown",
			/*8*/"Unknown",
			/*9*/"The AI must play this animation for as long as this timer lasts. Typically used on landing animations.",
			/*10*/"Calls a predefined attack perimeter && effects from the PAX",
			/*11*/"Combo period, applies a combo if you attack.",
			/*12*/"Plays a trail effect on the keyblade during an animation.",
			/*13*/"Utilized infinitely on Sora's Glide. Other uses unknown.",
			/*14*/"When an RC appears, allows the player to activate it.",
			/*15*/"Unknown",
			/*16*/"If the AI's current function allows it, uses a special mobility enhancement.",
			/*17*/"The AI checks the target's position, then determines if it can continue its combo or quits.",
			/*18*/"The AI checks the target's position, then determines if it can continue its alternate combo or quits.",
			/*19*/"The AI resets its resistances to default to remove invincibility frames.",
			/*20*/"Shows a certain reaction command on a certain bone from current model.",
			/*21*/"Unknown",
			/*22*/"The AI rotates to its target. If Sora uses a Reaction Command, he rotates to the enemy that gave it to him.",
			...
			/*26*/"Unknown",
			/*27*/"The player/AI cannot be hit by anything.",
			/*28*/"Rotates current model to his target.",
			/*29*/"You cannot fall off edges during the animation.",
			/*30*/"Unknown",
		...
		List<string> labels_EC2_explanations = new string[] {
			...
			/*7*/"The AI activates a special projectile or alternate effect such as slowing down.",
			...
			/*17*/"Unknown",
			/*18*/"Unknown",
			/*19*/"Acknowledge a contact between the map the the model",
			/*20*/"Unknown",
			/*21*/"Unknown",
			/*22*/"Unknown",
			...
			try
			{
				...
				try
				{
					...
					if (continuer)
					{
                        ...
                        Console.WriteLine(longueur_new_ec);
                        Console.WriteLine(AdjustBy16(33));
                        Console.WriteLine(AdjustBy16(31));
						ectobar_ = GetEcToBar(location_EC_cut);
						if (ectobar_!=-1)
						RemoveECLine(ectobar_/16);
						AddECLine(AdjustBy16(longueur_new_ec)/16);
						ec_file_bytes = SubArray(ec_file_bytes,32,longueur_new_ec);
