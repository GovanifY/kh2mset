...
namespace kh2mset
{
	public partial class MainForm
	{
		...
		void Label_lightgreenClick(object sender, EventArgs e)
		{
			MessageBox.Show("Light Green Slots are \"ANB\" slots.\r\n"+
			                "If the application finds a pointed ANB, the slot gets light green.\r\n"+
			                "Deleting a Light Green slot makes it turn to a \"DUMMY\" slot.\r\n\r\n"+
			                ...
			                "they will all turn into \"DUMMY\".","Color: Light Green", MessageBoxButtons.OK,MessageBoxIcon.Information);
		}
		...
		{
			blink_preview.BackColor = Color.FromArgb(Math.Min(Math.Max((int)SystemColors.Control.R, 0), (int)255),
                    Math.Min(Math.Max((int)SystemColors.Control.G-blink[0], 0), (int)255),
                    Math.Min(Math.Max((int)SystemColors.Control.B-blink[0], 0), (int)255));
			blink[0]+=5*blink[1];
